    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .hero { position: relative; width: 100vw; min-height: 88vh; overflow: hidden; }
        .header { display: flex; justify-content: space-between; align-items: center; padding: 20px 50px; position: absolute; top: 0; width: 100%; z-index: 10; }
        .logo { width: 100px; }
        .nav a { color: #fff; text-decoration: none; margin-left: 20px; font-size: 14px; }
        .nav a:hover { color: #dc3545; }
        .hero::before { content: ""; position: absolute; top: -10%; left: -5%; width: 110%; height: 130%; background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url(${heroBg}) no-repeat center center / cover; transform: skewY(-5deg); transform-origin: top left; z-index: 1; }
        ...
        .form-group input, .form-group select { width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px; background: #fff; }
        .form-group select { cursor: pointer; }
        .form-group.birthday select { width: 32%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px; background: #fff; }
        .form-group.birthday select:disabled { background: #f0f0f0; cursor: not-allowed; }
        .login-button { width: 100%; background-color: #dc3545; color: #fff; padding: 12px; border: none; border-radius: 20px; cursor: pointer; margin-top: 5px; font-size: 16px; font-weight: 600; }
        ...
        @media (max-width: 900px) { .hero-content { flex-direction: column; text-align: center; } .register-section { margin-top: 30px; width: 90%; max-width: 360px; } }
      `}</style>
          ...
          <div className="register-section" ref={registerBoxRef}>
            ...
            <form className="form-scroll" onSubmit={handleRegister}>
              ...
              <div className="form-group">
                ...
                <input type="text" name="fullName" value={formData.fullName} onChange={handleChangeExtra} required />
              </div>

              <div className="form-group birthday">
                <label>Birthday</label>
                <select name="month" value={formData.month} onChange={(e) => setFormData({ ...formData, month: e.target.value, day: "" })} required>
                  <option value="">Month</option>
                  {months.map((m) => <option key={m} value={m}>{m}</option>)}
                </select>
                <select name="day" value={formData.day} onChange={handleChangeExtra} disabled={!formData.month || !formData.year} required>
                  <option value="">Day</option>
                  {Array.from({ length: getDaysInMonth(formData.month, formData.year) }, (_, i) => i + 1).map((d) => <option key={d} value={d}>{d}</option>)}
                </select>
                <select name="year" value={formData.year} onChange={(e) => setFormData({ ...formData, year: e.target.value, day: "" })} required>
              ...
              <div className="form-group">
                ...
                <select name="security_question" value={formData.security_question} onChange={handleChangeExtra}>
                  <option value="">Select a security question (optional)</option>
                  {securityQuestions.map((question, index) => <option key={index} value={question}>{question}</option>)}
                </select>
              </div>
      ...
      <footer className="footer">
        <p><a href="/privacy" className="footer-link">Privacy Policy</a> | <a href="/terms" className="footer-link">Terms & Conditions</a></p>
        <div>Follow us on: <span className="social-icons">
          <a href="https://www.facebook.com/share/1as5kdEkMr/" target="_blank" rel="noopener noreferrer"><img src={facebookIcon} alt="Facebook" /></a>
          <a href="https://www.instagram.com/hoopersfits.ph?igsh=ZTFtNmw1YTR0OGZ6" target="_blank" rel="noopener noreferrer"><img src={instagramIcon} alt="Instagram" /></a>
        </span></div>
      </footer>
    </>
  );
